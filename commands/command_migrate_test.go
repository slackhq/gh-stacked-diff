...
func TestSdMigrate_EndsOnMainBranch(t *testing.T) {
	...
	currentBranch := util.ExecuteOrDieTrimmed(util.ExecuteOptions{}, "git", "branch", "--show-current")
	assert.Equal(util.GetMainBranchOrDie(), currentBranch)
}
...
func TestSdMigrate_SkipsDuplicateCommitsWhenMigratingBranchWithoutPR(t *testing.T) {
	...
	currentBranch := util.ExecuteOrDieTrimmed(util.ExecuteOptions{}, "git", "branch", "--show-current")
	assert.Equal(util.GetMainBranchOrDie(), currentBranch, "Should be on main branch after migration")
	...
	mainLog := util.ExecuteOrDie(util.ExecuteOptions{}, "git", "log", "--oneline", util.GetMainBranchOrDie())
